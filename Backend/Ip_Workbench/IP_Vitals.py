    ...
    if request.method == 'POST':
        try:
            ...
            vital_instance = IP_Vital_Details(
                ...
                Temperature_Status=temperature_status if temperature_status else None,
                Pulse_Rate=pulserate if pulserate else None,
                SPO2=spo2 if spo2 else None,
                SPO2_Status=spo2_status if spo2_status else None,
                Heart_Rate=heartrate if heartrate else None,
                Heart_Rate_Status=heartrate_status if heartrate_status else None,
                Respiratory_Rate=respiratoryrate if respiratoryrate else None,
                Respiratory_Status=respiratory_status if respiratory_status else None,
                SBP=sbp if sbp else None,
                SBP_Status=sbp_status if sbp_status else None,
                ...
                Painscore=Painscore if Painscore else None,
                SupplementalOxygen=SupplementalOxygen if SupplementalOxygen else None,
                SupplementalOxygen_Status=SupplementalOxygen_status if SupplementalOxygen_status else None,
                LevelOfConsiousness=LevelOfConsiousness if LevelOfConsiousness else None,
                LevelOfConsiousness_Status=LevelOfConsiousness_status if LevelOfConsiousness_status else None,
                CapillaryRefillTime=CapillaryRefillTime if CapillaryRefillTime else None,
                CapillaryRefillTime_Status=CapillaryRefillTime_status if CapillaryRefillTime_status else None,
                Type=Type if Type else None,
                DepartmentType=DepartmentType if DepartmentType else None,
        ...
        try:
            ...
            for idx, vital in enumerate(vital_details, start=1):
                vital_dict = {
                    ...
                    'PrimaryDoctorId': (
                        vital.Ip_Registration_Id.PrimaryDoctor.Doctor_ID 
                        if vital.Ip_Registration_Id and vital.Ip_Registration_Id.PrimaryDoctor 
                        else vital.Casuality_Registration_Id.PrimaryDoctor.Doctor_ID 
                        if vital.Casuality_Registration_Id and vital.Casuality_Registration_Id.PrimaryDoctor 
                        else None
                    ),
                    'PrimaryDoctorName': (
                        vital.Ip_Registration_Id.PrimaryDoctor.ShortName 
                        if vital.Ip_Registration_Id and vital.Ip_Registration_Id.PrimaryDoctor 
                        else vital.Casuality_Registration_Id.PrimaryDoctor.ShortName 
                        if vital.Casuality_Registration_Id and vital.Casuality_Registration_Id.PrimaryDoctor 
                        else None
                    ),
