...
useEffect(()=>{
  ...
  if(POdata && Object.keys(POdata).length !==0 && Object.keys(Editdata).length === 0)
  {
    ...
    if(POdata.Item_Details.length !==0){
      ...
      let FilterStatus=POdata.Item_Details.filter((ele)=>ele.Item_Status === 'Waiting For GRN')

      setPoItemlist(FilterStatus)
    }
    else{
      setPoItemlist([])
    }
...
useEffect(()=>{
  ...
  if(Editdata && Object.keys(Editdata).length !==0 && Object.keys(POdata).length === 0){
    ...
    axios.get(`${UrlLink}Inventory/PurchaseOrder_Link?PONumber=${Editdata?.PONumber}`)
    .then((res)=>{
      console.log('lllll',res.data); 
      let Pdatd = res.data
      if(Array.isArray(Pdatd)){

        let ItemPO=Pdatd[0].Item_Details
        setPoItemlist(ItemPO)
      }
      else{
        setPoItemlist([])
      }
    ...
    setGRNState({
      GRNNumber:Editdata.id,
      GRNDate: Editdata.GRNDate || currentDate,
      PONumber: Editdata.PONumber || '',
      SupplierCode: Editdata.SupplierCode || '',
      SupplierName: Editdata.SupplierName || '',
      SupplierBillNumber: Editdata.SupplierBillNumber || '',
      SupplierBillDate: Editdata.SupplierBillDate || '',
      SupplierBillDueDate: Editdata.SupplierBillDueDate || '',
      SupplierBillAmount: Editdata.SupplierBillAmount || '',
      SupplierBillLocation: Editdata.SupplierBillLocation || '',
      GRNLocation: Editdata.GRNLocation || '',
      ReceivingPersonName: Editdata.ReceivingPersonName || ''
    });

    setSupplierBillfile(Editdata.SupplierBillfile || null)

    setAmountstate({
      TaxableTotal:Editdata.TaxableTotal || '',
      TaxTotal:Editdata.TaxTotal || '',
      TotalAmount:Editdata.TotalAmount || '',
      TotalDiscountMethod:Editdata.TotalDiscountMethod || '',
      TotalDiscountType:Editdata.TotalDiscountType || '',
      TotalDiscount:Editdata.TotalDiscount || '',
      FinalTaxableTotal:Editdata.FinalTaxableTotal || '',
      FinalTaxTotal:Editdata.FinalTaxTotal || '',
      FinalTotalAmount:Editdata.FinalTotalAmount || '',
      RoundOff:Editdata.RoundOff || '',
      NetAmount:Editdata.NetAmount || '',
    })
    if(Editdata.GRN_Items.length !==0){
    setGRNItemlist(Editdata.GRN_Items)
    }
    else{
      setGRNItemlist([])
    }
...
const HandeleOnchange=(e)=>{
  ...
  if(name === 'SupplierBillNumber'){
    ...
    const paymentTermsDays = SingleSupplier.PaymentTerms;  
    const supplierBillDate = new Date(value);
...
useEffect(() => {
  if (Amountstate.TotalDiscountMethod === 'BeforeTax') {
    ...
    setAmountstate((prev) => ({
      ...prev,
      FinalTaxableTotal: TotalNet.toFixed(2),
      FinalTaxTotal: TtaxAmount.toFixed(2),
    ...
    setAmountstate((prev) => ({
      ...
      FinalTaxableTotal: (+Amountstate.TaxableTotal).toFixed(2),
      FinalTaxTotal: (+Amountstate.TaxTotal).toFixed(2),
      FinalTotalAmount: (+Amountstate.TotalAmount).toFixed(2),
      RoundOff: SecConv1.toFixed(2),
      NetAmount: OneConv1,
    }));

  }
}, [Amountstate.TotalDiscount, Amountstate.TotalDiscountMethod, Amountstate.TotalDiscountType]);
...
const SaveGRNfunc =()=>{
  ...
  if(SupplierField.length !== 0) {
    ...
    Senddata.append('PoItemlist', JSON.stringify(PoItemlist));
    ...
    if (SupplierBillfile) {      
      Senddata.append('SupplierBillfile', SupplierBillfile);
    }
  ...
  if(GRNItemStateFields.length !== 0) {
    ...
    if(CheckDublicate){
      ...
      const AdditemFun=()=>{
        ...
        console.log('checking--log---',GRNItemState.PendingQty,typeof(GRNItemState.PendingQty));
            ...
            Object.keys(GRNState).map((StateName,Index)=>{
              return(
                <div className="RegisForm_1" key={Index+'key'}>
                  ...
                  {
                   StateName ==='GRNLocation' || StateName ==='SupplierBillLocation' ?
              ...
              return(
                ...
                <div className="RegisForm_1" key={Index+'key'}>
                 ...
                 disabled={(GRNItemState.DiscountMethod === '' && StateName2 === 'DiscountType') ||
                  (GRNItemState.TotalPackTaxableAmount === '' && StateName2 === 'DiscountMethod') || (Amountstate.TotalDiscountMethod !== "" && StateName2 === 'DiscountMethod')}
                 ...
                 readOnly={(!['PackMRP','PurchaseRatePerPackTaxable','TaxType','TaxPercentage','PurchaseRatePerPack','ReceivedQty',
                  'BatchNo','ManufactureDate','ExpiryDate','DiscountMethod','DiscountType','Discount'].includes(StateName2))}  
              ...
              return(
                ...
                <select
                 ...
                 disabled={(StateName3 === 'TotalDiscountType' && Amountstate.TotalDiscountMethod === '') ||
                  ...
                  ) || (GRNItemlist.length === 0 && StateName3 === 'TotalDiscountMethod')
                 }             
                 >
                  <option value=''>Select</option>
                  {
